...
Character :: Character(Character const& copy) : name(copy.getName() + "copy")
{
	for (int i = 0; i < 5; i++)
	{
		if ((copy.inventory)[i])
	...
	std :: cout << "A character named " << name << " was created from copy of " << copy.name << std :: endl;
}
...
void Character :: equip(AMateria* m)
{
	...
	if (i >= 5)
	{
		std :: cout << this->name << " can't equip more than 5 Materia";
		return;
	}
	(this->inventory)[i] = m;
...
void Character :: use(int idx, ICharacter& target)
{
	...
	if (idx < 0 || idx >= 5 || !(this->inventory)[idx])
	{
